...
def create_base():
    ...
    try:
        with sqlite3.connect("nw_gui_dict.db", autocommit=False) as connection:
...
def fill_nw_context(name):
    """ Fill nw_context table with context name"""

    try:
        with sqlite3.connect("nw_gui_dict.db", autocommit=False) as connexion:
            # Auto-validation disabled (recommended for Python 3.12 and later)
...
def fill_nw_translation(translation):
...
def make_translation_tuple(context_list, root):
    ...
    for item in context_list:
        ...
        fill_nw_translation(translation_list)
